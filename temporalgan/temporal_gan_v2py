...
class Generator(nn.Module):
    ...
    def __init__(self, s2_in_channels=3,s1_in_channels = 1,out_channels=1, features=64):
        ...
        # Downsample blocks 
        self.pam_init  = PAM(features) # input from the initial_down layer / output goes to the down1 layer
        self.down1 = Block(features    , features * 2, down=True, act="leaky", use_dropout=False) # 64
        self.pam1 = PAM(features * 2) # input from the down1 layer / output goes to the down2 layer
        ...
        self.down6 = Block(features * 8, features * 8, down=True, act="leaky", use_dropout=False) # 2 * 512
