...
## Version 2.2: This just 1.2 but the s2 branch has down sampling like the orginal pix2pix.
"""
...
class Generator(nn.Module):
    ...
    def __init__(self, s2_in_channels=3,s1_in_channels = 1,out_channels=1, features=64):
       ...
       # Initial downsampling layer for S2
        ...
        self.up5 = Block((features * 8 * 2 + features * 4), features * 4, down=False, act="relu", use_dropout=False)
        self.up6 = Block(features * 4 * 2 +  features * 2, features * 2, down=False, act="relu", use_dropout=False)
        self.up7 = Block(features * 2 * 2 + features, features    , down=False, act="relu", use_dropout=False)

        # Final upsampling layer
        self.final_up = nn.Sequential(
            nn.Conv2d(features * 2, out_channels, OUTPUT_KSP[0], OUTPUT_KSP[1], OUTPUT_KSP[2]),
            nn.Tanh(),
        )
    def forward(self, s2: torch.Tensor , s1:torch.Tensor) -> torch.Tensor:
        ...
        d7 = torch.cat([d6_s2, d7_s1], 1)
        # Bottleneck
        bottleneck = self.bottleneck(d7)
        # Now we do the decoding part
        up1 = self.up1(bottleneck)
        up2 = self.up2(torch.cat([up1, d7], 1))
        up3 = self.up3(torch.cat([up2, d5_s2, d6_s1], 1))
        up4 = self.up4(torch.cat([up3, d4_s2, d5_s1], 1))
        up5 = self.up5(torch.cat([up4, d3_s2, d4_s1], 1))
        up6 = self.up6(torch.cat([up5, d2_s2, d3_s1], 1))
        up7 = self.up7(torch.cat([up6, d1_s2, d2_s1], 1))
        # Final upsampling which outputs the final image with only 1 channel.
        return self.final_up(torch.cat([up7, d1_s1], 1))
