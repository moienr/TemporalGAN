...
from gen_v1_2 import Generator as Generator_v1_2
...
class Generator(Generator_v1_2):
    def __init__(self, s2_in_channels=3, s1_in_channels=1, out_channels=1, features=64, num_reduced_channels=32):
        ...
        self.glam = GLAM(in_channels=features * 8 * 2, num_reduced_channels=num_reduced_channels, feature_map_size=8,kernel_size=5)
    ...
    def forward(self, s2: torch.Tensor , s1:torch.Tensor) -> torch.Tensor:
        ...
        d6 = torch.cat([d6_s2, d6_s1], 1)
        # Channel Attention
        # d6 = self.ca(d6) # Channel Attention to learn the importance of each stream, before fusing them.
        d6 = self.glam(d6)
        ...
        up3 = self.up3(torch.cat([up1, d6_s2, d6_s1], 1))
